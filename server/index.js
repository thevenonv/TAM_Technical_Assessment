...
app.use("/api/webhooks/paypal", express.raw({ type: "application/json" })); // raw for signature verify
...
function sendError(res, err) {
  ...
  res
    .status(status)
    .json({ error: err.message || "Internal error", debugId: err.debugId, details: err.data, hint: err.hint });
}
const asyncHandler = (fn) => (req, res, next) => Promise.resolve(fn(req, res, next)).catch((err) => sendError(res, err));
...
async function fetchCaptureRefunds(captureId) {
  ...
  try {
    ...
    if ((e.status || 0) === 404) return { refunds: [], captureStatus: null, captureAmount: null, captureCurrency: null, debugId: e.debugId };
    throw e;
  }
...
app.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!itemTotal || !shippingValue) {
      return res.status(400).json({ error: 'Missing itemTotal or shippingValue. Example: { "itemTotal":"10.00", "shippingValue":"4.99" }' });
    }
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!webhookId) return res.status(200).json({ ok: true, warning: "PAYPAL_WEBHOOK_ID missing" });
    ...
    if (verification?.verification_status !== "SUCCESS") return res.status(200).json({ ok: true, notVerified: true });
    ...
    const amount =
      resource.amount?.value ?? resource.seller_receivable_breakdown?.gross_amount?.value ?? null;
    const currency =
    ...
    if (
      ...
      try {
        ...
        const refunds =
          refundsData.refunds?.refunds || refundsData.refunds?.items || refundsData.refunds || [];
